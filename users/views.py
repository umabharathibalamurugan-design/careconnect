...
class RegisterView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # A resident needs a ResidentProfile before SOS/contact features can work.
        # If the registration request doesn't provide a flat, create a small demo
        # society/block/flat automatically so the extracted project is runnable.
        ...
        society, _ = Society.objects.get_or_create(
            ...
            defaults={
                'address': 'CareConnect Demo Campus',
                'city': 'Chennai',
                'pincode': '600001',
            },
        )
        ...
        if user.role in (User.Role.RESIDENT, User.Role.GUARDIAN):
            resident, _ = ResidentProfile.objects.get_or_create(
                user=user, defaults={'flat': flat, 'is_owner': user.role == User.Role.RESIDENT}
            )
            if user.role == User.Role.GUARDIAN:
                from guardians.models import Guardian
                Guardian.objects.get_or_create(
                    user=user, resident=resident, defaults={'relation': 'Family Guardian', 'is_primary': True}
                )
        elif user.role == User.Role.VOLUNTEER:
            from volunteers.models import Volunteer
            Volunteer.objects.get_or_create(
                user=user, society=society, defaults={'assigned_block': block, 'available_for_emergency': True}
            )
        elif user.role == User.Role.SECURITY:
            from security.models import SecurityGuard
            SecurityGuard.objects.get_or_create(
                user=user, society=society, defaults={'gate_assigned': 'Main Gate', 'is_on_duty': True}
            )
